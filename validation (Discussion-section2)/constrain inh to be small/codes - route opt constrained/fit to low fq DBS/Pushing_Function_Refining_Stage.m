function [opt_fit_FR_f,para_vec_full, sim_r_e_f, sim_r_i_f]  = Pushing_Function_Refining_Stage(ini_para_vec, r_D_0, r_I_0, FR_time_hist,I_DBS,opt_para)

%% Introduction

% Pushing Function in Refining Stage
...
weights_para =  ini_para_vec(2:5);

initial_other_paras = [ini_para_vec(1),ini_para_vec(6:10)];

para_vec = initial_other_paras;
...
for i = 2:length(FR_time_hist)
    ...
    Wr_e = weights_para(1)*(sim_r(i-1) + sim_r_e(i-1)) - weights_para(3)*sim_r_i(i-1);
    Wr_i = weights_para(2)*(sim_r(i-1) + sim_r_e(i-1)) - weights_para(4)*sim_r_i(i-1);
    
    dr(i) = (dt/para_vec(2))*(Wr_e-(sim_r(i-1)-r_D_0)+para_vec(3)/(1+exp(-para_vec(4)*(I_DBS(i)-para_vec(5)))));
    dr_e(i) = (dt/para_vec(2))*(Wr_e-(sim_r_e(i-1)-para_vec(6)));
...
%ratio_5 = 1; ratio_10 = 1;ratio_20 = 5;ratio_30 = 1;ratio_50 = 10;ratio_100 = 82;ratio_200 = 2500; 

% set the weight of high fq DBS data to 0
ratio_5 = 1; ratio_10 = 1;ratio_20 = 5;ratio_30 = 1;ratio_50 = 10;ratio_100 = 0;ratio_200 = 0; 
...
% if para_vec(1) < 2*para_vec(2) || para_vec(1) < i_constant_lower_limit ||  para_vec(2)< e_constant_lower_limit || para_vec(1) > i_constant_upper_limit || para_vec(2)> e_constant_upper_limit || para_vec(6) < baseline_FR_E_lower_limit || para_vec(6) > baseline_FR_E_upper_limit ||  para_vec(4) > 0.01
%     SSE_now = 1e222*SSE_now;
% end

% constraint that w_ei is less than 1/10 of the reported opt value
if para_vec(4) > (1/5)*opt_para(4) || para_vec(1) < 2*para_vec(2) || para_vec(1) < i_constant_lower_limit ||  para_vec(2)< e_constant_lower_limit || para_vec(1) > i_constant_upper_limit || para_vec(2)> e_constant_upper_limit || para_vec(6) < baseline_FR_E_lower_limit || para_vec(6) > baseline_FR_E_upper_limit ||  para_vec(4) > 0.01 
    SSE_now = 1e222*SSE_now;
end
...
for i = 2:length(FR_time_hist)
    ...
    Wr_e = weights_para(1)*(opt_fit_FR(i-1) + sim_r_e(i-1)) - weights_para(3)*sim_r_i(i-1);
    Wr_i = weights_para(2)*(opt_fit_FR(i-1) + sim_r_e(i-1)) - weights_para(4)*sim_r_i(i-1);
    
    dr(i) = (dt/para_vec(2))*(Wr_e-(opt_fit_FR(i-1)-r_D_0)+para_vec(3)/(1+exp(-para_vec(4)*(I_DBS(i)-para_vec(5)))));
    dr_e(i) = (dt/para_vec(2))*(Wr_e-(sim_r_e(i-1)-para_vec(6)));
    dr_i(i) = (dt/para_vec(1))*(Wr_i-(sim_r_i(i-1)-r_I_0));
...
% The full para_vec by including the fixed W:

para_vec_full = zeros(1, length(ini_para_vec));
para_vec_full(2:5) = ini_para_vec(2:5);
para_vec_full(1) = para_vec(1);
para_vec_full(6) = para_vec(2);
para_vec_full(7:10) = para_vec(3:6);

function [opt_fit_FR_f,para_vec, sim_r_e_f]  = Pushing_Function_Refining_Stage_p(ini_para_vec, r_D_0, FR_time_hist,I_DBS)

%% Introduction (reduced network without I group)

% Pushing Function for Refining Stage (w_ee is fixed)
...
% ini_para_vec = [Wee_0, tau_E_0, c_0, s_0, k_0, r_E_b_0]; initial parameters
% tau_E: excitatory time constant
% Wee: connectivity strengths (see paper)
...
% rate network model consists of 2 ODEs:
...
% r_D_0 = 25;  FR_time_hist = FR_time_hist_conc; I_DBS = I_DBS_conc;
% ini_para_vec = ini_para_network_model;
% ini_para_vec = ini_para;
% ini_para_vec = ref_ini_para;
...
for i = 2:length(FR_time_hist)
    ...
    Wr_e = ini_para_vec(1)*(sim_r(i-1) + sim_r_e(i-1));
    
    dr(i) = (dt/para_vec(1))*(Wr_e-(sim_r(i-1)-r_D_0)+ para_vec(2)/(1+exp(-para_vec(3)*(I_DBS(i)-para_vec(4)))));
    dr_e(i) = (dt/para_vec(1))*(Wr_e-(sim_r_e(i-1)-para_vec(5)));
...
% 
% 
% if  para_vec(1)< e_constant_lower_limit || para_vec(1)> e_constant_upper_limit  || para_vec(5) < baseline_FR_E_lower_limit || para_vec(5) > baseline_FR_E_upper_limit ||  para_vec(3) > 0.01
%     SSE_now = 1e222*SSE_now;
% end

e_constant_upper_limit = 100; 
...
% if  para_vec(1) < 0.1 || para_vec(2)< e_constant_lower_limit || para_vec(2)> e_constant_upper_limit  || para_vec(6) < baseline_FR_E_lower_limit || para_vec(6) > baseline_FR_E_upper_limit 
%     SSE_now = 1e222*SSE_now;
% end

if  para_vec(1) < 0.2 || para_vec(2)< e_constant_lower_limit || para_vec(2)> e_constant_upper_limit  || para_vec(6) < baseline_FR_E_lower_limit || para_vec(6) > baseline_FR_E_upper_limit 
    SSE_now = 1e222*SSE_now;
end
...
for i = 2:length(FR_time_hist)
    ...
    Wr_e = ini_para_vec(1)*(opt_fit_FR(i-1) + sim_r_e(i-1));    
    dr(i) = (dt/para_vec(1))*(Wr_e-(opt_fit_FR(i-1)-r_D_0)+para_vec(2)/(1+exp(-para_vec(3)*(I_DBS(i)-para_vec(4)))));
    dr_e(i) = (dt/para_vec(1))*(Wr_e-(sim_r_e(i-1)-para_vec(5)));
...
para_vec = [ini_para_vec(1),para_vec];

%% Introduction
% route optimization in Refining Stage
...
% ini_para_vec: initial parameters going into Refining Stage


%% inputs

FR_time_hist = FR_time_hist_conc; I_DBS = I_DBS_conc_p;
r_I_0 = 5; % baseline firing rate of the inhibitory neural group
r_D_0 = 25; % baseline firing rate of Vim neurons
num_iter = 40; % # of iterations in Refining Stage

initial_para = para_vec_S; % "para_vec_S" is the final output of Global Stage (see Readme of this folder)
...
opt_fit_mx = zeros(sim_length, 2*num_iter);
NMSE_vec = zeros(1,2*num_iter);
para_mx = zeros(length(initial_para),2*num_iter);


para_vec_S = initial_para;
for i = 1:num_iter
    
    [opt_fit_W,para_vec_P, ~, ~]  = Pushing_Function_Refining_Stage(para_vec_S, r_D_0, r_I_0,FR_time_hist,I_DBS);
    ...
    [opt_fit_AP,para_vec_S, sim_r_e, sim_r_i]  = Stabilizing_Function_Refining_Stage(para_vec_P, r_D_0, r_I_0,FR_time_hist,I_DBS);
